...
from src.financial.rules.goal_progress_threshold_rule import (
...
from src.financial.rules.low_account_balance_rule import (
    LowAccountBalanceRule,
)
...
class RuleEngine:
    """Evaluates registered financial rules."""
    ...
    def evaluate(
        ...
        for rule in self._rules:
            recommendation = rule.evaluate(snapshot)

            if recommendation is None:
                continue

            if not recommendation.source_rule:
                recommendation.source_rule = rule.__class__.__name__
...
def create_default_rule_engine() -> RuleEngine:
    """Return the application's default rule engine."""
    engine = RuleEngine()
