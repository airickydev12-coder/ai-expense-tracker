...
DEBT_TO_ASSETS_HIGH_THRESHOLD = 0.50
...
class DebtRatioRule(FinancialRule):
    ...
    def evaluate(self, snapshot: dict) -> Recommendation | None:
        ...
        assets = snapshot["total_account_balance"] + snapshot["total_goal_progress"]
        ...
        if ratio >= DEBT_TO_ASSETS_HIGH_THRESHOLD:
            return Recommendation(
                priority=RecommendationPriority.HIGH,
                category=RecommendationCategory.DEBT,
                title="High Debt Ratio",
                message=(f"Your debt equals {ratio:.0%} of your tracked assets."),
                action=("Focus on reducing debt while continuing to build assets."),
            )
