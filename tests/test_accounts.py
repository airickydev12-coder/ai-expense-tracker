...
def test_account_invalid_id():
    with pytest.raises(ValueError):
        Account(id=0, name="Checking", account_type="Bank", balance=1500)


def test_account_empty_name():
    with pytest.raises(ValueError):
        Account(id=1, name="", account_type="Bank", balance=1500)


def test_account_empty_type():
    with pytest.raises(ValueError):
        Account(id=1, name="Checking", account_type="", balance=1500)