...
def test_record_monthly_review_stamps_generated_at(tmp_path):
    ...
    load_monthly_review_history(file_path)
    ...
    assert len(get_monthly_review_history()) == 1
...
def test_record_monthly_review_is_restored_after_reload(tmp_path):
    ...
    load_monthly_review_history(file_path)
    record_monthly_review(build_ok_review(), file_path=file_path)

    clear_monthly_review_history()
    assert get_monthly_review_history() == []

    load_monthly_review_history(file_path)

    reviews = get_monthly_review_history()
...
def test_get_monthly_review_history_returns_copy(tmp_path):
    ...
    returned_reviews = get_monthly_review_history()
