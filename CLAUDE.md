...
- **`requirements.txt` is UTF-16 encoded** (a quirk of how it was generated) — if editing it programmatically, decode/encode as UTF-16, not UTF-8, or you'll corrupt it. `black` was removed from it (it's dev-only, already pinned in `requirements-dev.txt`), as were 7 more dev-tool-only transitive deps (`iniconfig`, `mypy_extensions`, `packaging`, `pathspec`, `platformdirs`, `pluggy`, `pytokens` — confirmed via `pip show <pkg>`'s `Required-by` to be needed only by `black`/`pytest`, already correctly pinned in `requirements-dev.txt`). Several CLI/telemetry-adjacent packages (`fastapi-cli`, `fastapi-cloud-cli`, `typer`, `rich`, `sentry-sdk`, `detect-installer`, `fastar`, `rignore`) are still pinned directly though not strictly required to run `uvicorn src.api.main:app` — left as-is since removing them risks dropping a genuine transitive dependency for a minor size win. `click`/`colorama`/`Pygments` look similar but are genuinely needed (uvicorn depends on `click`, which needs `colorama`; `rich` depends on `Pygments`) — don't remove those without re-checking `pip show`'s `Required-by` first.
...
- **Business-rule thresholds shared between the recommendation engine (`src/financial/rules/*.py`) and the coach (`src/financial/coach/insights.py`) live in `src/financial/shared/thresholds.py`.** Don't hand-copy a threshold value into a second file — that's exactly how two real bugs happened (a debt-to-income formula divergence, a net-worth severity mismatch), both from the same value being independently redefined and silently drifting. Thresholds unique to one side (e.g. the coach's extra emergency-fund/debt-to-income tiers, which have no engine equivalent) stay local — only values both sides actually share belong in the shared module.
